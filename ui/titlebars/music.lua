...
playerctl:connect_signal("shuffle", function(_, shuffle)
	shufflebtn.markup = shuffle and helpers.colorizeText("󰒝", beautiful.pri)
		or helpers.colorizeText("󰒝", beautiful.fg)
end)
...
volslider:connect_signal("property::value", function(_, value)
	if value > 1 then
		naughty.notify({ message = "hello " .. value })
		playerctl:set(value / 100)
	end
	naughty.notify({ message = "spoti " .. value })
	playerctl:set_volume(value)
end)
		...
		:setup({

			{
				{
					{
						...
						minimize,
						spacing = dpi(8),
						layout = beautiful.titlebarType == "vert" and wibox.layout.fixed.vertical
				...
				},
				{
					{
						{
							font = beautiful.titlebarType == "vert" and beautiful.icofont .. " 0"
								or beautiful.icofont .. " 17",
				...
				layout = beautiful.titlebarType == "vert" and wibox.layout.align.vertical
